...
const getOrdersByUser = catchAsync(async (req, res) => {
  ...
  const filter = pick(req.query, []);
  const options = pick(req.query, ["sortBy", "limit", "page"]);
  const result = await orderService.queryOrders({ userId, ...filter }, options);
  res.status(httpStatus.OK).send(result);
});

const updateStatusById = catchAsync(async (req, res) => {
  const order = await orderService.updateStatusById(req.params.orderId, req.body.status);
  res.status(httpStatus.OK).send(order);
})

export { createOrder, getOrders, getOrdersByUser, getOrder, updateOrder, deleteOrder, updateStatusById };
